...
func GetFilesPath(fileUrl string) (path string, err error) {
	gType, err := g.Cfg().Get(context.Background(), "upload.type")
	if err != nil {
		return "", err
	}
	upType := gstr.ToLower(gType.String())

	gUpPath, err := g.Cfg().Get(context.Background(), "upload.local.UpPath")
	if err != nil {
		return "", err
	}
	upPath := gstr.Trim(gUpPath.String(), "/")
